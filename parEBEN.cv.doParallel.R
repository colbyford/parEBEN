...
parEBEN.cv.doParallel <- function (BASIS, Target, nFolds, Epis = "no", foldId = 0, prior = "binomial"){
  if(prior == "binomial"){
    cat("Parallel Empirical Bayesian Elastic Net Linear Model (w/ Binomial Prior), Epis: ", Epis, ";", nFolds, 
    ...
    foreach(i_alpha = 1:nAlpha) %dopar% {
      alpha = Alpha[i_alpha]
      cat("Testing alpha", i_alpha, "/", nAlpha, ":\t\talpha: ", 
          alpha, "\n")
      SSE1Alpha = matrix(1e+10, N_step, 2)
      foreach (i_s = 1:N_step) %dopar% {
        lambda = Lambda[i_s]
        min_index = which.min(SSE1Alpha[1:(i_s - 1), 1])
        previousL = SSE1Alpha[min_index, 1] + SSE1Alpha[min_index, 
                                                        2]
        cat("\tTesting step", step, "\t\tlambda: ", lambda, 
            "\n")
        foreach(i = 1:nFolds,.combine = rbind) %dopar% {
          ...
          SimF2fEB <- EBEN::EBelasticNet.Binomial(Basis.Train, 
                                            Target.Train, lambda, alpha, Epis, verbose = 0)
          M = length(SimF2fEB$weight)/6
          Betas <- matrix(SimF2fEB$weight, nrow = M, ncol = 6, 
                          byrow = FALSE)
          Mu = Betas[, 3]
          ...
          if (M == 1 && Betas[1, 1] == 0) {
            ...
            basisTest = matrix(rep(0, ntest * M), ntest, 
                               M)
            foreach(i_basis = 1:M) %dopar% {
            ...
            if (min(temp) < 1e-10) 
              temp[which(temp < 1e-10)] = 1e-05
            logL[i] = mean(Target.Test * log(temp/(1 + 
                                                     temp)) + (1 - Target.Test) * log(1/(1 + temp)))
          }
        }
        Likelihood[step, ] = c(alpha, lambda, mean(logL), 
                               sd(logL))
